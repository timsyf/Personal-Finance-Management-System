...
def create_financial_data_visualization_tab(notebook, user_id):
    ...
    create_pie_chart_subtab(sub_tab_2_frame, user_id)
...
def create_transaction_summary_subtab(sub_tab_frame, user_id):
    ...
    if not transactions:
        ...
        for transaction in transactions:
            category = transaction[5] or 'Uncategorized'  # Index 5 is category in transactions table
            amount = float(transaction[2])  # Index 2 is amount
...
def create_pie_chart_subtab(sub_tab_frame, user_id):
    transactions = get_all_transactions(user_id)
    ...
    if not transactions:
        ...
        categories = {}
        for transaction in transactions:
            category = transaction[5] or 'Uncategorized'  # Index 5 is category
            amount = float(transaction[2])  # Index 2 is amount
            if category in categories:
                categories[category] += amount
            else:
